...
def boot_mlmem(dataset, x, m1, y, cluster, 
                              m2=None, m3=None, 
    ...
    for _ in range(n_boot): # Uses the 'n_boot' parameter
        ...
        indirect_effects = {}  # Store effects for current iteration
        ...
        boot_indirect_effects.append(indirect_effects)
        ...
        boot_direct_effects.append(direct_effects)
        ...
        boot_contrasts.append(contrasts)
    
    # 6. Calculate 95% CIs
    
    # Confidence Interval Calculation (Percentile-Based)
    indirect_cis = {}
    for key, values in boot_indirect_effects:
        for effect_key, effect_values in values.items():
            lower = np.percentile(effect_values, 2.5)
            upper = np.percentile(effect_values, 97.5)
            indirect_cis.setdefault(key, {})[effect_key] = (lower, upper)

    direct_cis = {} 
    for key, values in boot_direct_effects:
        for effect_key, effect_value in values.items():
            lower = np.percentile(effect_value, 2.5)
            upper = np.percentile(effect_value, 97.5)
            direct_cis.setdefault(key,{})[effect_key] = (lower, upper)

    contrast_cis = {} 
    for key, values in boot_contrasts:
        for effect_key, effect_value in values.items():
            lower = np.percentile(effect_value, 2.5)
            upper = np.percentile(effect_value, 97.5)
